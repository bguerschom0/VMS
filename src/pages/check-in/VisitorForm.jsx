...
const Alert = ({ message, type = 'error', onClose, onConfirm }) => (
  <motion.div
    ...
    className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 
      ...
      ${type === 'error' ? 'bg-red-500' : 'bg-black dark:bg-white'} 
...
const CheckOut = () => {
  ...
  const fetchVisitors = async () => {
    try {
      ...
      showAlert('Failed to fetch visitors', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Show alert
  const showAlert = (message, type = 'error', confirmAction = null) => {
    setAlert({
      message,
      type,
      confirmAction
  ...
  const confirmCheckout = async (visitorId) => {
    try {
      ...
      showAlert('Failed to check out visitor', 'error');
    }
  };
      ...
      <AnimatePresence>
        {alert && (
          <Alert 
            ...
            onConfirm={alert.confirmAction ? handleCloseAlert : null}
          />
        )}
      </AnimatePresence>
